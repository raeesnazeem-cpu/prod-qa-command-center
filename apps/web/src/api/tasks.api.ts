...
export const assignTask = async (
  ...
  const { data } = await axios.post<Task>(`/api/tasks/${id}/assign`, {
    user_id: userId,
  })
  return data
}
...
export const addComment = async (
  ...
  const { data } = await axios.post<TaskComment>(
...
export const addRebuttal = async (
  ...
  const { data } = await axios.post<TaskRebuttal>(
    `/api/tasks/${taskId}/rebuttals`,
    fullData,
  )
  return data
}
...
export const updateFindingStatus = async (
  ...
  const { data } = await axios.patch(`/api/findings/${findingId}/status`, {
    status,
  })
  return data
}
...
export const bulkPushToBasecamp = async (
  ...
  const { data } = await axios.post<{ basecampUrl: string; count: number }>(
    `/api/tasks/basecamp/bulk-push`,
    { taskIds },
  )
  return data
}
...
export const bulkPushCommentsToBasecamp = async (
  ...
  const { data } = await axios.post<{
    ...
    count: number
    skipped: number
  }>(`/api/tasks/basecamp/bulk-comment`, { taskIds, status })
  return data
}
...
export const getBasecampPeople = async (
  ...
  const { data } = await axios.get<
    Record<number, { sgid: string; name: string }>
  >(`/api/basecamp/people`, {
    params: { projectId },
  })
  return data
