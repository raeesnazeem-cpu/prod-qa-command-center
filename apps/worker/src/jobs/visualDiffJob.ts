...
async function downloadBuffer(
  ...
  try {
    ...
    logger.warn(
      ...
      "Failed to download image buffer",
    )
    return undefined
  }
}
...
export async function processVisualDiffJob(job: Job) {
  ...
  if (!run.figma_url) {
    logger.warn(
      { runId },
      "No Figma URL provided for this run, skipping visual diff",
    )
    return
  }
  ...
  const frames = await exportFigmaFrames(
    run.figma_url,
    figmaToken,
    supabase,
    runId,
  ...
  if (!frames || frames.length === 0) {
    logger.error(
      { figmaUrl: run.figma_url },
      "Figma exporter returned no frames",
    )
    throw new Error(
      "No Figma frames could be extracted from the provided URL. Please verify the URL and Figma token.",
    )
  }
  ...
  try {
    ...
    logger.warn(
      { url: page.url },
      "Invalid page URL, using / as path for matching",
    )
  }

  logger.info(
    { normalizedPagePath, frameCount: frames.length },
    "Matching Figma frames to page path",
  )

  // Try exact match first
  let matchedFrame = frames.find(
    (f: FigmaFrame) => f.pageUrl === normalizedPagePath,
  )

  // Try partial match if no exact match
  if (!matchedFrame) {
    matchedFrame = frames.find(
      (f: FigmaFrame) =>
        normalizedPagePath.includes(f.pageUrl) && f.pageUrl !== "/",
    )
  }
  ...
  if (!matchedFrame && frames.length > 0) {
    logger.warn(
      { normalizedPagePath },
      "No exact or partial frame match found, falling back to first available frame",
    )
  ...
  if (!matchedFrame) {
    throw new Error(
      `No Figma frames found to compare for path: ${normalizedPagePath}. Available frame paths: ${frames.map((f: FigmaFrame) => f.pageUrl).join(", ")}`,
    )
  }

  // Step 5: Download Figma PNG and Site desktop screenshot
  logger.info(
    { matchedFrame: matchedFrame.frameName, frameUrl: matchedFrame.pageUrl },
    "Downloading images for comparison...",
  )
  ...
  if (signedSiteError || !signedSite?.signedUrl) {
    logger.warn(
      { pageId, error: signedSiteError?.message },
      "Failed to generate fresh signed URL for site screenshot, falling back to DB URL",
    )
  }

  const siteBuffer = await downloadBuffer(
    signedSite?.signedUrl || page.screenshot_url_desktop,
  )
  ...
  const diffResult = await queueGeminiCall(() =>
    compareScreenshots(figmaBuffer, siteBuffer, page.url),
  )
  ...
  if (diffResult.issues.length > 0) {
    ...
    if (findingsError) {
      logger.error(
        ...
        "Failed to create findings from visual diff",
      )
    }
  }

  logger.info(
    { pageId, issuesFound: diffResult.issues.length },
    "Visual diff job completed successfully",
  )
}
