  ...
  try {
    ...
    if (data.figma_token_encrypted) {
      try {
        ...
        console.error(
          `Failed to decrypt figma_token for project ${projectId}:`,
          e,
        )
    ...
    if (data.basecamp_token_encrypted) {
      try {
        ...
        console.error(
          `Failed to decrypt basecamp_token for project ${projectId}:`,
          e,
        )
    ...
    console.error(
      `Error fetching decrypted settings for project ${projectId}:`,
      error,
    )
    throw error
  }
