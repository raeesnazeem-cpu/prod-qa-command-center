...
function getRelevantTools(messages: ChatMessage[], allTools: any[]) {
  ...
  const categories = {
    ...
    rag:
      historyText.includes("search") ||
      ragKeywords.some((k) => historyText.includes(k)),
  }
  ...
  return allTools.filter((t) => {
    ...
    // Stats tools
    if (
      t.name.includes("_stats") ||
      t.name.includes("_status") ||
      t.name.includes("_issues_by_")
    ) {
    ...
    try {
      ...
      if (isNoEndpoints || isRateLimit || isNotFound) {
        logger.warn(
          ...
          `OpenRouter model ${model} unavailable, trying next...`,
        )
  ...
  for (const msg of messages) {
    ...
    if (msg.role === "user") {
      ...
      if (
        lastTurn &&
        lastTurn.role === "user" &&
        lastTurn.parts[0].functionResponse
      ) {
        lastTurn.parts.push({
          functionResponse: {
            name: msg.name,
  ...
  while (rounds < MAX_ROUNDS) {
    ...
    for (const model of modelsToTry) {
      try {
        ...
        logger.warn(
          { model, error: err.message },
          `Gemini provider model ${model} failed, trying next...`,
        )
        continue
      }
    }
    ...
    if (!calls || calls.length === 0) {
      return {
        content: response.text || parts.map((p: any) => p.text).join(""),
        history: [],
        usage: totalUsage,
      }
    }
...
async function cohereChat(
  ...
  return openAiCompatibleChat(
    "Cohere",
    "https://api.cohere.com/v1/compatibility/openai/v1",
