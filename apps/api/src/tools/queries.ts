...
export async function getIssueCountsByQA(projectId: string) {
  ...
  const runMap = new Map(
    runs.map((r) => {
      // Safely handle both array and object returns just in case
      const userObj: any = Array.isArray(r.users) ? r.users[0] : r.users
      return [r.id, userObj?.full_name || userObj?.email || "Unknown"]
    }),
  )
