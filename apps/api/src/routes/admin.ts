 ...
 */
router.post('/ping-worker', clerkAuth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/queue-stats', clerkAuth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const counts = await qaQueue.getJobCounts('waiting', 'active', 'completed', 'failed', 'delayed');
 ...
 */
router.get('/redis-info', clerkAuth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const redis = qaQueue.opts.connection;
    ...
    const estimatedCost = (totalCommands / 100000) * 0.20;
    ...
    const now = new Date();
    if (!lastStats || (now.getTime() - new Date(lastStats.created_at).getTime()) > 3600000) {
      await supabase.from('redis_stats').insert({ total_commands: totalCommands });
    }
    ...
    const commands24h = stats24h 
      ? (totalCommands < stats24h.total_commands ? totalCommands : (totalCommands - stats24h.total_commands)) 
      : 0;
    ...
    const commandsMonth = statsMonth 
      ? (totalCommands < statsMonth.total_commands ? totalCommands : (totalCommands - statsMonth.total_commands)) 
      : totalCommands;

    const totalMonthCost = (Math.max(0, commandsMonth) / 100000) * 0.20;
    const total24hCost = (Math.max(0, commands24h) / 100000) * 0.20;
    ...
    const calculateRunCosts = (runs: any[] | null) => {
      return (runs || []).reduce((acc, run) => {
        ...
        return acc + (estimatedCommands / 100000) * 0.20;
      }, 0);
    };
    ...
    const runStats = (recentRuns || []).map(run => {
      ...
      return {
        ...
        cost: (estimatedCommands / 100000) * 0.20
      };
    });
 ...
 */
router.get('/run-history', clerkAuth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    ...
    const history = (runs || []).map(run => {
      ...
      return {
        ...
        estimated_cost: (estimatedCommands / 100000) * 0.20
      };
    });
 ...
 */
router.get('/activity-logs', clerkAuth, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const { page = 1, limit = 50, search = '', entityType = '', startDate = '', endDate = '', projectName = '' } = req.query;
 ...
 */
router.delete('/activity-logs', clerkAuth, requireRole('super_admin'), async (req: Request, res: Response) => {
  try {
    // Delete all rows
