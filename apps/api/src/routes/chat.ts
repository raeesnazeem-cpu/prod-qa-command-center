    ...
    try {
      ...
      const executeToolCall = async (name: string, args: any) => {
        ...
        const result = await (async () => {
          switch (name) {
            case "find_project":
              return await queries.findProjectByName(
                args.project_name || args.projectName,
                orgId!,
              )
            case "find_projects_bulk":
              return await queries.findProjectsByNames(
                args.project_names || args.projectNames,
                orgId!,
              )
            case "get_project_stats":
              return await queries.getProjectStats(projectId!)
            case "get_task_stats":
              return await queries.getTaskStats(projectId!)
            case "get_developers":
              return await queries.getDevelopersForProject(projectId!)
            case "get_qa_engineers":
              return await queries.getQAForProject(projectId!)
            case "get_project_members":
              return await queries.getProjectMembers(projectId!)
            case "get_project_status":
              return await queries.getProjectPreReleaseStatus(projectId!)
            case "get_basecamp_link":
              return await queries.getProjectBasecampLink(projectId!)
            case "get_issues_by_developer":
              return await queries.getIssueCountsByDeveloper(projectId!)
            case "get_issues_by_qa":
              return await queries.getIssueCountsByQA(projectId!)

            case "get_all_users":
              return await queries.getAllOrgUsers(orgId!)
            case "find_user":
              return await queries.getUserByEmail(args.email, orgId!)
            case "find_user_by_name":
              return await queries.findUserByName(args.name, orgId!)
            case "get_user_tasks":
              return await queries.getTasksByUserId(userId)
            case "get_user_task_stats":
              return await queries.getUserTaskStats(userId)
            case "get_org_task_stats":
              return await queries.getOrgTaskStats(orgId!)
            case "list_projects":
              return await queries.listProjects(orgId!)
            case "get_user_projects":
              return await queries.getUserProjects(userId)

            case "create_project":
              return await mutations.createProject(args, orgId!, performer)
            case "update_project":
              return await mutations.updateProject(
                { ...args, project_id: projectId },
                orgId!,
                performer,
              )
            case "delete_project":
              return await mutations.deleteProject(
                { ...args, project_id: projectId },
                orgId!,
                performer,
              )
            case "delete_projects_bulk":
              return await mutations.deleteProjectsBulk(
                { ...args, project_ids: args.project_ids || args.projectIds },
                orgId!,
                performer,
              )
            case "add_project_member":
              return await mutations.addProjectMember(
                { ...args, project_id: projectId, user_id: userId },
                performer,
              )
            case "remove_project_member":
              return await mutations.removeProjectMember(
                { ...args, project_id: projectId, user_id: userId },
                performer,
              )
            case "create_task":
              return await mutations.createTask(
                {
                  ...args,
                  project_id: projectId,
                  assigned_to: userId || args.assignedTo,
                },
                orgId!,
                performer,
              )
            case "update_task":
              return await mutations.updateTask(
                {
                  ...args,
                  task_id: taskId,
                  project_id: projectId,
            ...
            case "delete_task":
              return await mutations.deleteTask(
                { ...args, task_id: taskId, project_id: projectId },
                performer,
              )
            case "delete_tasks_bulk":
              return await mutations.deleteTasksBulk(
                { ...args, task_ids: args.task_ids, project_id: projectId },
                performer,
              )
            case "delete_user_tasks_in_project":
              return await mutations.deleteUserTasksInProject(
                { ...args, user_id: userId!, project_id: projectId! },
                performer,
              )
            case "update_finding":
              return await mutations.updateFinding(
                { ...args, finding_id: findingId, run_id: runId },
                orgId!,
                performer,
              )
            case "delete_finding":
              return await mutations.deleteFinding(
                { ...args, finding_id: findingId, run_id: runId },
                performer,
              )
            ...
            case "create_qa_run":
              return await mutations.createRun(
                { ...args, project_id: projectId },
                performer,
              )
            case "cancel_qa_run":
              return await mutations.cancelRun(
                { ...args, run_id: runId, project_id: projectId },
                performer,
              )

            case "search_issues":
              return await ragSearch.semanticSearch(
                args.query,
                orgId!,
                projectId,
                args.source_type || args.sourceType,
              )
...
- Keep responses short and focused.`
      ...
      const result = await chatWithFallback(
        ...
        (provider, stats) => {
          ...
          res.write(
            `data: [METADATA]${JSON.stringify({ intermediate: true, provider, stats })}\n\n`,
          )
        },
      )
      ...
      res.write(
        `data: [METADATA]${JSON.stringify({ provider, failedProviders, allStats })}\n\n`,
      )
